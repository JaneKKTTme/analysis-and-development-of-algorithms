...
n=2000 #max число элементов
v_original=[] #начальный массив, хранилище, не меняется (если все способы прогоняем через один и тот же массив)
v=[] #рабочий массив, перед каждым новым пунктом берем из хранилища начальные значения (если для каждого пункта рандомим новый массив, то эту переменную убрать)

#заполнение хранилица
for i in range(n):
